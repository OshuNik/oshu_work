...
(function () {
  ...
  // ✅ FIX: Refresh favorites when a vacancy status is updated (e.g., liked from main page)
  document.addEventListener('vacancy:updated', (e) => {
    console.debug('[Favorites] Vacancy updated event received, refreshing...', e.detail);
    loadFavorites();
  });

  // ✅ FIX: Also listen to realtime updates
  document.addEventListener('realtime:ready', () => {
    console.log('[Favorites] Realtime ready, can now sync updates');
  });
