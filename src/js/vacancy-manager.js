...
(function() {
  ...
  class VacancyManager {
    ...
    /**
     * Initialize loaded IDs tracking for a category
     */
    initializeCategory(key) {
      if (!this.loadedIds.has(key)) {
        this.loadedIds.set(key, new Set());
      }
    ...
    /**
     * Clear loaded IDs when refetching category
     */
    clearLoadedIds(key) {
      this.loadedIds.set(key, new Set());
    }
    ...
    /**
     * Check if vacancy ID was already loaded
     */
    isVacancyLoaded(key, vacancyId) {
      return this.loadedIds.has(key) && this.loadedIds.get(key).has(vacancyId);
    }

    /**
     * Mark vacancy as loaded
     */
    markVacancyLoaded(key, vacancyId) {
      this.initializeCategory(key);
      this.loadedIds.get(key).add(vacancyId);
    }

    /**
     * Mark all vacancies as loaded
     */
    markVacanciesLoaded(key, vacancyIds) {
      this.initializeCategory(key);
      const idSet = this.loadedIds.get(key);
      vacancyIds.forEach(id => idSet.add(id));
    }
    ...
    async processVacanciesResult(key, result, isInitialLoad, isPullToRefresh = false) {
      ...
      // Clear loaded IDs on initial load
      if (state.offset === 0) {
        this.clearLoadedIds(key);
      }
      ...
      const validItems = items.filter(item => {
        ...
        if (this.isVacancyLoaded(key, item.id)) {
          console.warn(`[VacancyManager] Duplicate detected: ${item.id}`);
          return false;
        }
      ...
      // Mark all loaded vacancies
      this.markVacanciesLoaded(key, validItems.map(item => item.id));
    ...
    async performSwipeStatusUpdate(vacancyId, newStatus, isFavorite) {
      ...
      // Функция отмены с transition-анимацией въезда
      const onUndo = () => {
        console.log('🔄 Отмена свайпа: Анимация + Замена', vacancyId);
        ...
        setTimeout(() => {
          try {
            ...
            if (newCard) {
              ...
              console.log('✅ Карточка заменена после анимации.');
            } else {
              throw new Error('Не удалось создать новую карточку');
            }
          } catch (error) {
            console.error('Ошибка в onUndo:', error);
    ...
    // Анимация скрытия карточки для кнопок
    animateCardHiding(cardElement) {
      // БЕЗ transform - только плавное исчезновение
