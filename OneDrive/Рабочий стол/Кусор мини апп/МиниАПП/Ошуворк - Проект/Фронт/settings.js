...
(function() {
  ...
  const loadSelectedPresetBtn = document.getElementById('loadSelectedPresetBtn');
  ...
  const defaultPresets = [
    {
      title: 'Дизайн',
      tags: ['Photoshop', 'Illustrator', 'Figma', 'UI/UX', 'Веб-дизайн', 'Графика']
    },
    {
      title: 'Разработка',
      tags: ['JavaScript', 'React', 'Node.js', 'Python', 'Full-stack', 'Frontend']
    },
    {
      title: 'Маркетинг',
      tags: ['SMM', 'Контекстная реклама', 'SEO', 'Аналитика', 'Контент', 'PR']
    },
    {
      title: 'Копирайтинг',
      tags: ['Тексты', 'Реклама', 'SMM', 'SEO', 'Лендинги', 'Email-рассылки']
    },
    {
      title: 'Видео',
      tags: ['Монтаж', 'After Effects', 'Premiere Pro', 'Motion Design', 'Анимация']
    },
    {
      title: '3D',
      tags: ['Blender', '3ds Max', 'Maya', 'ZBrush', 'Substance', 'Рендеринг']
    }
  ];
  ...
  // Создание сетки пресетов
  function createPresetsGrid() {
    presetsGrid.innerHTML = '';
    defaultPresets.forEach((preset, index) => {
      const presetCard = document.createElement('div');
      presetCard.className = 'preset-card';
      presetCard.dataset.index = index;
      
      presetCard.innerHTML = `
        <div class="preset-title">${preset.title}</div>
        <div class="preset-tags">${preset.tags.slice(0, 3).join(', ')}${preset.tags.length > 3 ? '...' : ''}</div>
      `;
      
      presetCard.addEventListener('click', () => selectPreset(index));
  ...
  function selectPreset(index) {
    ...
    const selectedCard = presetsGrid.querySelector(`[data-index="${index}"]`);
    if (selectedCard) {
  ...
  function loadSelectedPreset() {
    if (selectedPreset === null) {
      safeAlert('Выберите пресет для загрузки');
      return;
    }
    
    const preset = defaultPresets[selectedPreset];
    ...
    // Показываем уведомление (заглушка для базы данных)
    safeAlert(`Пресет "${preset.title}" загружен! (База данных пока не готова)`);
  }

  // Загрузка случайного пресета
  function loadRandomPreset() {
    const randomIndex = Math.floor(Math.random() * defaultPresets.length);
    const preset = defaultPresets[randomIndex];
    keywords = [...preset.tags];
    renderKeywords();
    ...
    // Показываем уведомление (заглушка для базы данных)
    safeAlert(`Случайный пресет "${preset.title}" загружен! (База данных пока не готова)`);
  }

  // Открытие модального окна пресетов
  function openPresetModal() {
    presetModal.classList.add('active');
    createPresetsGrid();
  }

  // Закрытие модального окна пресетов
  function closePresetModal() {
    presetModal.classList.remove('active');
    selectedPreset = null;
  }

  // Открытие модального окна сохранения пресета
  function openSavePresetModal() {
    if (keywords.length === 0) {
      safeAlert('Сначала добавьте ключевые слова для сохранения');
      return;
    }
  ...
  // Закрытие модального окна сохранения пресета
  ...
  function savePreset(event) {
    ...
    if (!presetName) {
      safeAlert('Введите название пресета');
      return;
    }
    ...
    // Показываем уведомление (заглушка для базы данных)
    safeAlert(`Пресет "${presetName}" сохранен! (База данных пока не готова)`);
  }
  ...
  function clearAllKeywords() {
    if (keywords.length === 0) {
      safeAlert('Нет ключевых слов для удаления');
      return;
    }
  ...
  function renderKeywords() {
    if (keywords.length === 0) {
      keywordsTagsContainer.innerHTML = '<div class="no-keywords">Нет активных ключевых слов</div>';
      return;
    }
    
    keywordsTagsContainer.innerHTML = keywords.map((keyword, index) => `
      <div class="keyword-tag">
        ${escapeHtml(keyword)}
        <button class="keyword-remove-btn" onclick="window.removeKeyword(${index})" title="Удалить">×</button>
      </div>
    `).join('');
  }
   ...
   */
  function initPage() {
    console.log('settings.js: Инициализация страницы...');
    ...
    console.log('settings.js: Страница инициализирована');
  }
  ...
  function initKeywords() {
    ...
    if (loadPresetBtn) {
      loadPresetBtn.addEventListener('click', openPresetModal);
    }
    ...
    if (clearAllKeywordsBtn) {
      clearAllKeywordsBtn.addEventListener('click', clearAllKeywords);
    }
    
    // Обработчики для модальных окон
    if (closeModal) {
      closeModal.addEventListener('click', closePresetModal);
    }
    
    if (closeSaveModal) {
      closeSaveModal.addEventListener('click', closeSavePresetModal);
    }
    ...
    if (randomPresetBtn) {
      randomPresetBtn.addEventListener('click', loadRandomPreset);
    }
    
    if (loadSelectedPresetBtn) {
      loadSelectedPresetBtn.addEventListener('click', loadSelectedPreset);
    }
