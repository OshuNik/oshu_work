# Улучшенная система уведомлений для парсера - ГОТОВО К ИНТЕГРАЦИИ
# Замените ваши текущие функции уведомлений на эти
...
async def send_telegram_notification(vacancy_data: dict, user_settings: dict):
    """Отправляет расширенное уведомление через Telegram Bot API с кнопками"""
    ...
    try:
        # Формируем расширенное сообщение и кнопки
...
1. Замените в вашем parser.py функции уведомлений:
   - Старую send_telegram_notification() → на новую send_telegram_notification()
   - Старую format_notification_message() → на новую format_enhanced_notification_message()

2. Добавьте новые функции извлечения данных:
   - extract_salary_info()
   - extract_company_name()
   - extract_location_info()

3. Добавьте функцию создания кнопок:
   - create_inline_keyboard()

4. Опционально добавьте обработку callback'ов:
   - handle_callback_query()

5. Импорты которые нужны:
   - import urllib.parse (для URL encoding)
   - import datetime (для времени)

РЕЗУЛЬТАТ:
✅ Умные уведомления с извлечением зарплаты, компании и локации из текста
✅ Красивое форматирование с приоритетом важной информации  
✅ URL кнопки для Mini App, поиска, настроек и перехода к сообщению
✅ Интерактивные кнопки "В избранное" и "Отключить на час"
✅ Совместимость с вашей текущей системой настроек
"""